...
public class NotasController {
    ...
    public ResponseEntity<List<Notas>> listarTodasNotas() {
     ...
     // Actualizar nota
    @PutMapping("/{id}")
    public ResponseEntity<Notas> actualizarNota(
            @PathVariable Long id,
            @RequestBody Notas notaActualizada) {
        
        Notas nota = notasService.actualizarNota(
                id,
                notaActualizada.getNombre(),
                notaActualizada.getContenido(),  
                notaActualizada.getRecursoActivo()
        );
        return ResponseEntity.ok(nota);
