package Testing.ApoloNotes.unitarias;
...
class NotasServiceTest {
    ...
    void crearNota_DeberiaGuardarNotaSiDatosSonValidos() {
        ...
        String listag = null; // nuevo parámetro

        Notas notaMock = new Notas(nombre, listag, fecha, contenido);
        when(notasRepository.save(any(Notas.class))).thenReturn(notaMock);

        Notas resultado = notasService.crearNota(nombre, listag, fecha, contenido);
    ...
    void crearNota_DeberiaLanzarExcepcionSiNombreEsNulo() {
        ...
        IllegalArgumentException exception = assertThrows(
                ...
                () -> notasService.crearNota(nombre, listag, fecha, contenido)
        );
    ...
    // Búsqueda de nota existente
    @Test
    void obtenerNotaPorId_DeberiaRetornarNotaSiExiste() {
        Long id = 1L;
        Notas notaMock = new Notas("Nota encontrada", null, LocalDateTime.now(), "Contenido");
        notaMock.setId(id);
    ...
    void eliminarNota_DeberiaMarcarNotaComoInactiva() {
        Long id = 1L;
        Notas notaMock = new Notas("Nota a eliminar", null, LocalDateTime.now(), "Contenido");
        notaMock.setId(id);
    ...
    // Eliminación de nota inexistente
    @Test
